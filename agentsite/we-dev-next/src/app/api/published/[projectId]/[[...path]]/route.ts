...
import { RELEASES_DIR } from '../../../paths';
...
function transformHtml(html: string, projectId: string) {
  const basePath = `/api/published/${encodeURIComponent(projectId)}/`;
...
export async function GET(_req: Request, { params }: RouteContext) {
  try {
    if (!PROJECT_ID_PATTERN.test(params.projectId)) {
      return Response.json({ error: 'Invalid projectId' }, { status: 400 });
    }
    const projectDirectory = join(RELEASES_DIR(), params.projectId);
    const currentPath = join(projectDirectory, 'current.json');
    if (!existsSync(currentPath)) return new Response('Published release not found', { status: 404 });
    const current = JSON.parse(await readFile(currentPath, 'utf8')) as { releaseId?: string };
    if (!current.releaseId || !RELEASE_ID_PATTERN.test(current.releaseId)) {
      return new Response('Published release pointer is invalid', { status: 503 });
    }

    const segments = safeSegments(params.path);
    const releaseDirectory = join(projectDirectory, current.releaseId);
    ...
    const candidates = [
      ...
      join(releaseDirectory, ...relativeSegments),
      join(releaseDirectory, 'public', ...relativeSegments),
      join(distDirectory, ...assetSegments),
      join(releaseDirectory, 'public', ...assetSegments),
    ];
    let filePath = '';
    ...
    if (!filePath || !await existingFile(filePath)) {
      return new Response('Published asset not found', { status: 404 });
    }
    ...
    const commonHeaders = {
      ...
      'X-We0-Release': current.releaseId,
    };

    if (extension === '.html') {
      return new Response(transformHtml(await readFile(filePath, 'utf8'), params.projectId), {
        headers: { ...commonHeaders, 'Content-Security-Policy': "frame-ancestors 'self'" },
      });
    }
    const data = await readFile(filePath);
    return new Response(data, {
      headers: { ...commonHeaders, 'Content-Length': String(data.byteLength) },
    });
  } catch (error) {
